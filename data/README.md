...
To enable publishing, commit `.github/workflows/bunker.yml` and the source files on `main` using credentials permitted to update workflows, set repository secret `API_FOOTBALL_KEY` (and optional repository variable `BUNKER_LEAGUES`), enable Actions with write access to repository contents, and configure GitHub Pages to serve the `gh-pages` branch root. The hourly cron (UTC) then fetches, validates, rebuilds and replaces the static Pages contents automatically when two picks qualify. `npm run build:static` exports to `out/`; the workflow verifies `out/index.html` and `out/bunker/index.html` before publishing `./out` (not `./public`). No provider credentials are stored in the static site or repository. The generated client JSON is public: **never put API tokens or private data in source fields**.
