    ...
    return (
      <main className="min-h-screen">
        ...
        <section className="mx-auto flex min-h-[70vh] max-w-xl flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-3 max-w-sm text-sm leading-7 text-slate-400">השלימו דרופ ושמרו את תוצאת הזכייה במכשיר הזה כדי לפתוח את מעבדת Einstein Drop.</p>
      ...
      <section className="mx-auto w-full max-w-6xl px-4 pb-14 pt-8 sm:px-8 sm:pt-14">
        ...
        >
          ...
          <div className="relative flex flex-col items-start justify-between gap-7 sm:flex-row sm:items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-300/25 bg-emerald-300/[0.07] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-200"><ShieldCheck size={13} /> DROP COMPLETE · ACCESS GRANTED</div>
              <p className="mt-5 flex items-center gap-2 text-sm font-bold text-cyan-200"><BrainCircuit size={18} /> Einstein Drop · Intelligence Desk</p>
              <h1 className="mt-2 text-4xl font-black tracking-tight text-white sm:text-6xl">THE <span className="bg-gradient-to-l from-cyan-200 via-cyan-400 to-lime-300 bg-clip-text text-transparent">BUNKER</span></h1>
              <p className="mt-3 text-lg font-bold text-slate-200">הבנקר · סקירת עומק אנליטית</p>
              <p className="mt-2 max-w-xl text-sm leading-7 text-slate-400">סקירת שוק קדם-משחק של שתי בחירות Over 2.5, חישוב יחסים, ומסגרת בדיקה לנתונים הדרושים לפני גיבוש תחזית.</p>
            </div>
            <motion.div
          ...
          <div className="relative mt-7 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.08] pt-4 text-[10px] font-semibold text-cyan-100/60">
            <span className="flex items-center gap-2"><motion.span className="h-2 w-2 rounded-full bg-emerald-300 shadow-[0_0_12px_rgba(52,211,153,.8)]" animate={{ opacity: [.45, 1, .45] }} transition={{ duration: 1.8, repeat: Infinity }} />ANALYST BRIEF · SLIP INPUTS</span>
