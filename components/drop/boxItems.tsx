"use client";

import {
  BallIcon,
  CardIcon,
  ChipIcon,
  CrestIcon,
  KingIcon,
  StackIcon,
} from "./sportsIcons";

export type RarityName = "common" | "uncommon" | "rare" | "classified" | "covert" | "special";
export type ItemCategory = "bonus" | "cash";
export type ItemIconName = "crest" | "ball" | "chip" | "card" | "stack" | "king";

// Emoji glyphs shown on the card-reveal front faces
export const ITEM_EMOJI: Record<ItemIconName, string> = {
  crest: "👑",
  ball: "⚽",
  chip: "🪙",
  card: "🃏",
  stack: "💵",
  king: "🤴",
};

export interface RarityStyle {
  label: string;
  color: string;
  bg: string;
  border: string;
  glow: string;
}

// Pure black & gold casino palette — rarity reads as golden intensity, never neon.
export const RARITIES: Record<RarityName, RarityStyle> = {
  common: {
    label: "נפוץ",
    color: "#98a2b3",
    bg: "rgba(152,162,179,0.07)",
    border: "rgba(152,162,179,0.45)",
    glow: "rgba(152,162,179,0.25)",
  },
  uncommon: {
    label: "לא נפוץ",
    color: "#c9a25a",
    bg: "rgba(201,162,90,0.09)",
    border: "rgba(201,162,90,0.55)",
    glow: "rgba(201,162,90,0.32)",
  },
  rare: {
    label: "נדיר",
    color: "#e0b03f",
    bg: "rgba(224,176,63,0.1)",
    border: "rgba(224,176,63,0.6)",
    glow: "rgba(224,176,63,0.36)",
  },
  classified: {
    label: "מסווג",
    color: "#eec154",
    bg: "rgba(238,193,84,0.1)",
    border: "rgba(238,193,84,0.65)",
    glow: "rgba(238,193,84,0.4)",
  },
  covert: {
    label: "סודי",
    color: "#f6d479",
    bg: "rgba(246,212,121,0.1)",
    border: "rgba(246,212,121,0.7)",
    glow: "rgba(246,212,121,0.46)",
  },
  special: {
    label: "מיוחד",
    color: "#ffc93c",
    bg: "rgba(255,201,60,0.12)",
    border: "rgba(255,201,60,0.8)",
    glow: "rgba(255,201,60,0.55)",
  },
};

export interface BoxItem {
  id: string;
  name: string;
  category: ItemCategory;
  icon: ItemIconName;
  amount: number | string;
  chance: string;
  weight: number;
  rarity: RarityName;
}

// Exact rewards from the community WhatsApp drop — football & poker themed
export const BOX_ITEMS: BoxItem[] = [
  {
    id: "bonus-40",
    name: "40% בונוס לא מקוזז בהפקדה הבאה",
    category: "bonus",
    icon: "crest",
    amount: "40%",
    chance: "24%",
    weight: 24,
    rarity: "classified",
  },
  {
    id: "cash-100",
    name: "100 מתנה בהפקדה הבאה",
    category: "cash",
    icon: "stack",
    amount: 100,
    chance: "6%",
    weight: 6,
    rarity: "covert",
  },
  {
    id: "bonus-50",
    name: "50% בונוס לא מקוזז בהפקדה הבאה",
    category: "bonus",
    icon: "ball",
    amount: "50%",
    chance: "30%",
    weight: 30,
    rarity: "uncommon",
  },
  {
    id: "cash-50",
    name: "50 שקל מתנה בהפקדה הבאה",
    category: "cash",
    icon: "card",
    amount: 50,
    chance: "12%",
    weight: 12,
    rarity: "rare",
  },
  {
    id: "cash-150",
    name: "150 שקל מתנה בהפקדה הבאה",
    category: "cash",
    icon: "king",
    amount: 150,
    chance: "2%",
    weight: 2,
    rarity: "special",
  },
  {
    id: "cash-20",
    name: "20 שקל מתנה בהפקדה הבאה",
    category: "cash",
    icon: "chip",
    amount: 20,
    chance: "26%",
    weight: 26,
    rarity: "common",
  },
];

export function pickWeighted(items: BoxItem[]): BoxItem {
  const total = items.reduce((sum, item) => sum + item.weight, 0);
  let roll = Math.random() * total;
  for (const item of items) {
    if (roll < item.weight) return item;
    roll -= item.weight;
  }
  return items[items.length - 1];
}

export function ItemIcon({
  icon,
  size = 32,
  className,
}: {
  icon: ItemIconName;
  size?: number;
  className?: string;
}) {
  switch (icon) {
    case "crest":
      return <CrestIcon size={size} className={className} />;
    case "ball":
      return <BallIcon size={size} className={className} />;
    case "chip":
      return <ChipIcon size={size} className={className} />;
    case "card":
      return <CardIcon size={size} className={className} />;
    case "stack":
      return <StackIcon size={size} className={className} />;
    case "king":
      return <KingIcon size={size} className={className} />;
  }
}